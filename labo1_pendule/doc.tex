...
\begin{document}
        ...
        \begin{minipage}{0.45\textwidth}
            ...
            \fbox{%
                \begin{tabular}{rcl}
                    ...
                    T(s) & - & période en secondes \\
                    $\Delta$T^2(s^2) & - & incertitude de T^2 \\
                \end{tabular}%
            }
        \end{minipage}
    \end{table} \\
    On peut utiliser cet ensemble de données pour faire un graphique: \\
    \\
    \pgfplotstableread[col sep=comma]{plotData/plotExpII.csv}\expIdata
    \begin{tikzpicture}[scale=1]
        % Scatter plot
        \begin{axis}[
            xlabel={$L$},
            ylabel={$T^2$},
            legend pos=north east,
            legend style={at={(0.35,0.75)}, anchor=south},
            grid=both,
            width=1\textwidth,
            height=0.3\textheight,
            x tick label style={
            ...
            \addplot+[only marks, mark=x, error bars/.cd, x dir=both, x explicit, y dir=both, y explicit] table[x=L, x error=err_L, y=T2, y error=err_T2] {\expIdata};

            % Linear regression line
            \addplot [red, thick] table[
                y={create col/linear regression={y=T2}}
            ] {\expIdata};
            \xdef\slope{\pgfplotstableregressiona}
            \xdef\intercept{\pgfplotstableregressionb}
        ...
        \end{axis}
    \end{tikzpicture} \\
    Après l'analyse du graphique, on constate plusieurs choses:
    \begin{itemize}
        \item L'imprécision des mesures augmente de plus en plus quand les valeurs augmentent (à cause de $T^2$)
        \item On a, en effet, une dépendance linéaire de $\sqrt{L}$ pour la période (ou une dépendance linéaire de L pour $T^2$)
        \item Avec la pente obtenue, on peut estimer l'accélération gravitationnelle de la terre
    \end{itemize}
    Pour calculer l'accélération gravitationnelle, on procède à l'approche mathématique suivante (on appelle \textit{a} la pente de $T^2(L)$):
    \begin{align*}
        g=\frac{4\pi^2 L}{T^2} &\Rightarrow g=\frac{4\pi^2}{a} \\
        g&\approx9.54ms^{-2}
    \end{align*}
    On estime $\Delta a=\pm0.01$, alors:
    \begin{align*}
        g=\frac{4\pi^2}{a} &\Rightarrow \Delta g=4\pi^2 \Delta a \\
        \Delta g &= \pm0.40 ms^{-2}
    \end{align*}
    ...
    En outre, nous avons réussi à déduire la constante d'accélération gravitationnelle $(g=(9.5 \pm 0.4)ms^{-2})$ qui est très proche de la constante réelle $\approx 9.81$.
\end{document}
